...
class Struct2Seq(nn.Module):
    ...
    def forward(self, X, S, L, mask):
        ...
        V, E, E_idx = self.features(X, L, mask) # V: [32, 482, 128] E: [32, 482, 30, 128]  E_idx: [32, 482, 30]
        h_V = self.W_v(V) # [32, 482, 128]
        h_E = self.W_e(E) # [32, 482, 30, 128]

        # Encoder is unmasked self-attention
        mask_attend = gather_nodes(mask.unsqueeze(-1),  E_idx).squeeze(-1) # [32, 482, 30], 邻居节点不能是dummy node
        mask_attend = mask.unsqueeze(-1) * mask_attend # [32, 482, 30] # mask:标记中心节点是(0)或不是(1)dummy node
        for layer in self.encoder_layers:
            h_EV = cat_neighbors_nodes(h_V, h_E, E_idx) # [32, 482, 30, 256]
            h_V = layer(h_V, h_EV, mask_V=mask, mask_attend=mask_attend) # [32, 482, 128]

        # Concatenate sequence embeddings for autoregressive decoder
        h_S = self.W_s(S) # [32, 482] --> [32, 482, 128]
        h_ES = cat_neighbors_nodes(h_S, h_E, E_idx) # [32, 482, 30, 256]

        # Build encoder embeddings
        h_ES_encoder = cat_neighbors_nodes(torch.zeros_like(h_S), h_E, E_idx) # [32, 482, 30, 256]
        h_ESV_encoder = cat_neighbors_nodes(h_V, h_ES_encoder, E_idx) # [32, 482, 30, 384]

        # Decoder uses masked self-attention
        mask_attend = self._autoregressive_mask(E_idx).unsqueeze(-1) # [32, 482, 30, 1], 标记邻居节点的索引是(1)或否(0)严格小于中心节点,去除自环
        mask_1D = mask.view([mask.size(0), mask.size(1), 1, 1]) # [32, 482, 1, 1], 标记中心节点是(0)或不是(1)dummy node
        mask_bw = mask_1D * mask_attend # [32, 482, 30, 1], mask: lower_idx-->higher_idx, 使用h_ESV
        
        if self.forward_attention_decoder:
            mask_fw = mask_1D * (1. - mask_attend) # [32, 482, 30, 1], mask: higher_idx-->lower_idx，使用h_ESV_encoder(其中S初始化为0)
            h_ESV_encoder_fw = h_ESV_encoder # [32, 482, 30, 384]
        else:
            h_ESV_encoder_fw = 0
        for layer in self.decoder_layers:
            ...
            h_V = layer(h_V, h_ESV, mask_V=mask) # h_V: [1, 400, 128], h_ESV: [1, 400, 30, 384] # h_ESV-->计算attention权重; h_V-->节点特征向量, 作为query

        logits = self.W_out(h_V) # [32, 482, 20]
        log_probs = F.log_softmax(logits, dim=-1) # [32, 482, 20]
        return log_probs
